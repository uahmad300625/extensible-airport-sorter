...
@task
def run():
    sh('python src/airport_sorting_console_program.py')

@task
def test():
    sh('python -m coverage run --source src --omit src/airport_sorting_console_program.py,src/airport_data.py -m unittest discover -s test')
